...
router.patch('/:post_id', authenticate, function (req, res) {
  ...
  console.log("PATching: ", req.params.post_id, " with ", req.body);
  Post.findOneAndUpdate({_id: req.params.post_id}, req.body, { new: true, useFindAndModify:true } , function (err, post) {
    if (err) {
      console.log(err);
      res.status(500).json({
